	...
	export default Vue.extend({
		...
		methods: {
			...
			updateSliderPosition(event) {
				...
				const cursorXfromViewport = e.touches
					? e.touches[0].pageX
					: e.pageX;

				// Calc Cursor Position from the left edge of the window (consider any page scrolling)
				const cursorXfromWindow =
					cursorXfromViewport - window.pageXOffset;
		...
		computed: {
			...
			leftImageStyle() {
				return {
					clip: `rect(auto, ${this.imageWidth *
						this.sliderPositionPercentage}px, auto, auto)`
				};
			},
			rightLabelStyle() {
				const cutLeft = Math.max(
					...
					this.rightLabelWidth +
						this.imageWidth * (this.sliderPositionPercentage - 1)
				);
				return {
...
<style scoped lang="css">
	...
	.vci-slider .left-arrow {
		height: "0px";
		width: "0px";
	}

	.vci-slider .right-arrow {
		height: "0px";
		width: "0px";
	}
</style>
